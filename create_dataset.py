...
def add_datetime(df):
    ...
    for climber in (subdir_sien, subdir_tim):
        ...
        for climb_dir in climber.iterdir():
            ...
            with open(metadir.joinpath("time.csv"), "r") as datetime_file:
                ...
                start_time, end_time = (
                    datetime_df["system time text"][0],
                    datetime_df["system time text"][1],
                )
    ...
    for group_name, group_data in df_copy.groupby("entry_num"):
        ...
        start_datetime, end_datetime = pd.to_datetime(
            datetime_dict[group_name][0]
        ), pd.to_datetime(datetime_dict[group_name][1])
        datetime_range = pd.date_range(
            start=start_datetime, end=end_datetime, periods=len(group_data)
        )
